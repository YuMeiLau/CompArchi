...
void opcodeOp(char* lOpcode, char* lArg1, char* lArg2, char* lArg3, char* lArg4, int programCounter){
  int dr, sr1, sr2, imm5, n, z, p, pcOffset9, pcOffset11, baseR, bOffset6, offset6, trapVect8;
  int nzp;
  int insValue;
  int addrOfLabel;
  if(!strcmp(lOpcode, "add")){
    ...
    baseR = lArg2[1] - '0';
    offset6 = toNum(lArg3);
    if(offset6 > 31 || offset6 < -32) exit(2); /*invalid constant*/
    insValue = (6 & 15) << 12 | (dr & 7) << 9 | (baseR & 7) << 6 | (offset6 & 31);
    insertMachineCode(insValue);
  }

 else if(!strcmp(lOpcode, "trap")){
    trapVect8 = toNum(lArg1);
    if(trapVect8 > 255 || trapVect8 < 0) exit(2); /*invalid constants */
    ...
    if( lRet != DONE && lRet != EMPTY_LINE ){
      if(count == 0){
        ...
        if(!strcmp(lOpcode, ".orig")) exit(4); /* multiple .orig ?? what about multiple .end*/
          ...
          else if(isOpcode(lOpcode)) opcodeOp(lOpcode, lArg1, lArg2, lArg3, lArg4, programCounter);
          else exit(2);
        }
